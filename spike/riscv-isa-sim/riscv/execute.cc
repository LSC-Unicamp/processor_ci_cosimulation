...
static void commit_log_print_insn(processor_t *p, reg_t pc, insn_t insn)
{
  ...
  for (auto item : load) {
    ...
    // Align the memory address to a multiple of four //GABRIEL
    reg_t aligned_address = std::get<0>(item); // & ~0x3; // GABRIEL
    commit_log_print_value(log_file, xlen, aligned_address); //GABRIEL
